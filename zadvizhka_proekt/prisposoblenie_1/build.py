...
EAR_ROOT_Y = 55.0      # полуширина лап у корня: размер 110 (вид слева) - приоритет над видом сверху
EAR_TAN = (math.sqrt(106.0 ** 2 - EAR_ROOT_Y ** 2), EAR_ROOT_Y)  # корень кромки лапы на Ø212
...
CH_D = 5.0                     # Ø5*
CH_D2 = 4.0                    # ДОПУЩЕНИЕ: Ø4 - наклонные связи каналов вне плоскости разреза (в поз.12)
CH_D2Y = 4.8                   # ДОПУЩЕНИЕ: Ø4,8 - сверление по Y от вертикального канала
...
SPIG = (148.2, 29.95)          # Ø60 Js7/h7 - центрирующий поясок; 55* = 148,2 - 93,2
...
BOSS = (10.0, 27.0, 53.0, 51.5)  # прилив под штуцер: ±x, z0 (размер 27), z1, торец Y (вид слева, изм.)
D15 = (25.3, 6.3, 20.9, 25.9)  # шайба поз.15
NUT23 = (18.0, 6.0)            # гайка М12 ГОСТ 5916-70: S=18; ГОСТ m=7, по чертежу 6 (изм. 8,9..14,9..20,9)

# --- Верхний узел (главный разрез + 1,2 мм) -------------------------------
Z5 = (173.2, H_TOP, 192.0)     # поз.5: низ пояска, низ/верх фланца (15* -> 14, чтобы выдержать 84 и 2)
R5 = (49.0, 74.0, 56.0, 44.0)  # Ø98, Ø148, Ø112 Js6, Ø88 H8
...
ANG5 = (0, 120, 240)           # 3 винта: на разрезе только справа (x=+63), слева сплошная штриховка
PCD83 = 83.0                   # Ø83 - болты под планшайбой поз.12
ANG83 = (45, 135, 225, 315)    # 4 болта М6; на разрезе условно повёрнуты в плоскость (ГОСТ 2.305)
BOLT83_L = 32.0                # М6×32 ГОСТ 7798-70 (планшайба 30 + выступ ~2, как на чертеже)
Z_PIN8 = Z5[1] + 30.0 - 0.3    # штифт Ø8 H7/m8: 30±0,5 от плоскости корпуса
Z_PIV = 188.1                  # ось рычага Ø8 (изм. 186,9 + 1,2)
X_PIV = 22.0                   # 44±1
L_BLK = (43.95, 178.0, 198.0, 8.1, 37.2, 6.0)  # опора рычагов Ø88 f9 (87,9)
J = (56.1, 63.3, 194.0, 63.3)  # опора: x_in, x_out, z низ (зазор 2 к фланцу 192; 278-84=194), ±y
...
ST_HOLE_R = 56.0               # нижнее отверстие Ø112 (стенка обрывается на x=±56,1)
ST_HOLE_TOP_R = 50.0           # верхнее отверстие Ø100 (вид слева: хорда ≈100, глубина ≈20)
...
JAW_WRAP = (16.0, -63.3)       # торец -Y губок охватывает стакан снизу до x=±16 (32*, вид сверху)
SCR_END = (61.5, 233.0, 12.0)  # винты М3 в торце губок: x, z, длина (вид сверху x=±61,5)
SCR_SIDE = (50.0, 233.0, 67.9, 8.0)  # ДОПУЩЕНИЕ: 2-й винт М3×8 губки (сбоку, по X): y, z, |x| опоры головки, длина
PIN_AR = (62.0, 6.0, 16.0)     # ДОПУЩЕНИЕ: штифт 6m6×16 ГОСТ 3128-70 от проворота поз.12 в поз.7 (y, d, l)
...
def make_base():
    ...
    for sgn in (1, -1):
        ear = cq.Workplane("XY").polyline([
            (sgn * EAR_X, -EAR_W_TIP), (sgn * EAR_X, EAR_W_TIP), (sgn * (EAR_TAN[0] - 0.3), EAR_TAN[1]),
            (sgn * 60.0, EAR_TAN[1]), (sgn * 60.0, -EAR_TAN[1]), (sgn * (EAR_TAN[0] - 0.3), -EAR_TAN[1])
...
def make_housing():
    ...
    # отверстие под штифт от проворота оси поз.12 (допущение)
    s = s.cut(cylZ(PIN_AR[1] / 2, CAV1[1] - 1, CAV1[1] + PIN_AR[2] / 2, y=PIN_AR[0]))
...
def make_support_J(sg):
    ...
    s = s.cut(cylY(1.5, -hy - 1, -hy + SCR_END[2] - (JAW_WRAP[1] - JAW[4]) + 1.0, x=SCR_END[0], z=SCR_END[1]))  # М3
    s = s.cut(cylX(1.5, xo + 1, SCR_SIDE[2] - SCR_SIDE[3] - 1.0, y=SCR_SIDE[0], z=SCR_SIDE[1]))  # М3 (допущение)
    s = s.clean()
    return s if sg > 0 else mir(s)
...
def make_jaw(sg):
    ...
    # торцевая часть (-Y): охватывает стакан снизу до x=-16 (размер 32*, вид сверху)
    wrap = cylY(ro, y0, JAW_WRAP[1], z=Z_ST).cut(cylY(ri, y0 - 1, JAW_WRAP[1] + 1, z=Z_ST))
    wrap = wrap.intersect(box(-ro - 1, -JAW_WRAP[0], y0, JAW_WRAP[1], Z_ST - ST_R[0], Z_TOP_JAW))
    s = s.fuse(wrap)
    s = s.cut(cylY(1.7, y0 - 1, JAW_WRAP[1] + 1, x=-SCR_END[0], z=SCR_END[1]))
    # 2-й винт (допущение): отверстие Ø3,4 и площадка под головку
    s = s.cut(cylX(1.7, -SCR_SIDE[2] - 1, -J[1] + 1, y=SCR_SIDE[0], z=SCR_SIDE[1]))
    s = s.cut(cylX(3.1, -ro - 2, -SCR_SIDE[2], y=SCR_SIDE[0], z=SCR_SIDE[1]))
    s = s.clean()
    return s if sg < 0 else mir(s)
...
def make_spindle():
    ...
    # ДОПУЩЕНИЕ: связи вне плоскости разреза, чтобы обе бесштоковые полости были на проточке 1,
    # а обе штоковые - на проточке 2 (по Y от вертикального канала, затем наклонно к отрезку)
    for (xv, yv, xe, zc) in ((xl, -20.0, 66.0, CH_CAP_Z), (xr, 20.0, -66.2, CH_ROD_Z)):
        # сверление по Y Ø4,8 (чуть меньше Ø5 - без касания равных цилиндров)
        s = s.cut(cylY(CH_D2Y / 2, 0, yv + math.copysign(3.0, yv), x=xv, z=zc))
        p0, p1 = V(xv, yv, zc), V(xe, 0, zc)
        d = (p1 - p0).normalized()
        ps = p0 - d * 1.0          # торец наклонного сверления целиком внутри сверления по Y
        s = s.cut(cyl(CH_D2 / 2, (ps.x, ps.y, ps.z), (d.x, d.y, d.z), (p1 - ps).Length + 1.0))
    s = s.cut(cylZ(PIN_AR[1] / 2, fz1 - PIN_AR[2] / 2, fz1 + 1, y=PIN_AR[0]))
    # резьбовые отверстия М6 под болты на Ø83 (сквозные)
    for ang in ANG83:
        a = math.radians(ang)
        s = s.cut(cylZ(3.0, fz0 - 1, fz1 + 1, PCD83 / 2 * math.cos(a), PCD83 / 2 * math.sin(a)))
    return s.clean()
...
def make_stakan():
    ...
    s = s.cut(cylZ(ST_HOLE_R, Z_ST - 80, Z_ST))
    s = s.cut(cylZ(ST_HOLE_TOP_R, Z_ST, Z_ST + 80))
    return s.clean()
...
def build():
    ...
    for i, ang in enumerate(ANG83):
        a = math.radians(ang)
        p = (PCD83 / 2 * math.cos(a), PCD83 / 2 * math.sin(a), S12_FL[1])
        add("x_bolt_planshayby_%d" % (i + 1), "", "Болт М6-6g×32.58 ГОСТ 7798-70 - позиция не указана",
            bolt_hex(6.0, BOLT83_L, 10.0, 4.0, p, (0, 0, 1)), FAST, "Сталь 35", "ГОСТ 7798-70",
            "Окружность Ø83; головка под планшайбой поз.12, ввёрнут в планшайбу (как на чертеже)",
            group="x_bolt_planshayby")
    add("x_shtift_osi", "", "Штифт 6m6×16 ГОСТ 3128-70 - ДОПУЩЕНИЕ (на чертеже не показан)",
        cylZ(PIN_AR[1] / 2, S12_FL[2] - PIN_AR[2] / 2, S12_FL[2] + PIN_AR[2] / 2, y=PIN_AR[0]),
        FAST, "Сталь 45", "ГОСТ 3128-70",
        "Фиксирует ось поз.12 от проворота относительно корпуса поз.7 (совпадение воздушных каналов)")
    ...
    for i, ang in enumerate(ANG5):
        ...
        add("x_vint_flantsa_%d" % (i + 1), "", "Винт М6-6g×20.58 ГОСТ 1491-80 - позиция не указана",
            ...
            "ГОСТ 1491-80", "Крепление поз.5 к корпусу (Ø126), 3 шт. через 120°", group="x_vint_flantsa")
    ...
    for sd, sg in SIDES:
        ...
        add("x_vint_gubki_" + sd, "", "Винт М3-6g×12.58 ГОСТ 17473-80 - позиция не указана",
            screw_round(3.0, SCR_END[2], 5.5, 2.1, 0.8, 1.0, (sg * SCR_END[0], JAW[4], SCR_END[1]), (0, 1, 0)),
            FAST, "Сталь 35", "ГОСТ 17473-80", "Крепление торца губки поз.1 к опоре стакана (вид сверху, y=-70)",
            group="x_vint_gubki")
        add("x_vint_gubki_bok_" + sd, "", "Винт М3-6g×8.58 ГОСТ 17473-80 - ДОПУЩЕНИЕ (на чертеже не показан)",
            screw_round(3.0, SCR_SIDE[3], 5.5, 2.1, 0.8, 1.0, (sg * SCR_SIDE[2], SCR_SIDE[0], SCR_SIDE[1]),
                        (-sg, 0, 0)),
            FAST, "Сталь 35", "ГОСТ 17473-80",
            "Второе крепление губки поз.1 к опоре стакана со стороны +Y (y=50), принято конструктивно",
            group="x_vint_gubki_bok")
